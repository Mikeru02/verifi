...
class Tester(Command):
    ...
    def statistics_checker(self, start_time, stop_flag):
        while not stop_flag["stop"]:
            ...
            sys.stdout.write(f"\rRuntime: {elapse} seconds | Recent doc kb: {Statistics.get_recent_kb():.4f} kb | Recent time: {Statistics.get_recent_time():.4f} s")
            sys.stdout.flush()
            time.sleep(1)
        sys.stdout.write("\n")
    ...
    def execute(self) -> int:
        if self.__include_statistics:
            ...
            for document in self.__test_data[:self.__number_of_items]:
                ...
                prediction = Predictor(content, self.__class_count, self.__class_word_count, self.__vocab_size).execute()
